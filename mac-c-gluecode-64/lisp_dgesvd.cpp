...
void fortran_to_c_matrix(double *mf, double *m, int nrow, int ncol)
{
...
extern "C"
int lisp_dgesvd(double *a, int nrow, int ncol,
		 double *u, double *w, double *vt)
{
  ...
  int nrcmin = (nrow < ncol) ? nrow : ncol;
  ...
  try {
...

    {
      ...
      int nmin = (nrow > ncol) ? ncol : nrow;
      int nmax = (nrow > ncol) ? nrow : ncol;
      int na = 3*nmin+nmax;
      int nb = 5*nmin-4;
...
extern "C"
int lisp_dgesvd_bksb(double *u, double *w, double *vt, double *y,
		      double *x, int nrow, int ncol, double tolerance)
{
  if (tolerance < 0.0)
    tolerance = 0.0;
...
extern "C"
int lisp_dgesvd_solve(double *a, double *y, double *x, int nrow, int ncol,
		       double tolerance)
{
  double *u = 0;
  double *w = 0;
...
extern "C"
int lisp_dgesvd_predict(double *v, double *xv, int nel, int ncol,
			 double tolerance,
			 double *pred, int npred,
			 double *ll, double *hh,
			 double *sd)
{
  int err = 0;

  int nrowmax = nel - ncol;
  ...
  try
    {
      ...
      for(int nfut = 1; nfut <= npred; ++nfut)
	{
	  int nrow = nel - ncol - nfut + 1;
	  int row, col;
