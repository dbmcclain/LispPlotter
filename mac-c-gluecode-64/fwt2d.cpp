...
inline void erase_floats(float *dst, int offset, int nel)
{
  memset(dst+offset, 0, nel*sizeof(float));
}
  
void fill_floats(float *dst, int offset, float val, int nel)
{
  for(int ix = nel; --ix >= 0;)
    dst[offset+ix] = val;
}
...
void __TFWT2D_TransformerRep::implant_kernel(float *pkrnl, int nkrows, int nkcols)
{
  // use only from inside of critical section
...
float **create_output_buffer(int tsize)
{
...
__TFWT2D_TransformerRep::__TFWT2D_TransformerRep(int nirows, int nicols)
  : TObject()
{
  bare_initialize();
  try {
...
void __TFWT2D_TransformerRep::initialize(int nirows, int nicols)
{
  ...
  // as int as image size boundaries are respected
...
void __TFWT2D_TransformerRep::init_kernel(float *pkrnl, int nkrows, int nkcols)
{
  ...
  VDSP_CRITICAL_SECTION {
    ...
    for(int iy = m_nirows; --iy >= 0; )
      {
	int off = iy * m_nicols2;
	for(int ix = m_nicols2; --ix > 0; )
	  {
	    int   k  = off + ix;
    ...
    for(int iy = 2; iy < m_nirows; iy += 2)
      {
	int kre = iy*m_nicols;
	int kim = kre + m_nicols;
...
// --- testing ---
float* __TFWT2D_TransformerRep::get_fkernel(int *pnrows, int *pncols)
{
  // not critical
...
TFWT2D_Transformer::TFWT2D_Transformer(int nirows, int nicols)
  :THandle<__TFWT2D_TransformerRep>(new __TFWT2D_TransformerRep)
{
...
TFWT2D_Daub97_HL::TFWT2D_Daub97_HL(int nirows, int nicols)
  :THandle<__TFWT2D_TransformerRep>(new __TFWT2D_TransformerRep)
{
  initialize(nirows, nicols);
}

void TFWT2D_Daub97_HL::initialize(int nirows, int nicols)
{
  float krnl[KHEIGHT*KWIDTH];

  for(int iy = KHEIGHT; --iy >= 0;)
    for(int ix = KWIDTH; --ix >= 0; )
...
TFWT2D_Daub97_LH::TFWT2D_Daub97_LH(int nirows, int nicols)
  :THandle<__TFWT2D_TransformerRep>(new __TFWT2D_TransformerRep)
{
  initialize(nirows, nicols);
}

void TFWT2D_Daub97_LH::initialize(int nirows, int nicols)
{
  float krnl[KHEIGHT*KWIDTH];
...
extern "C"
{
  FWTSetup fwt2d_make_transformer(int nirows, int nicols)
  {
  ...
  FWTSetup fwt2d_make_daub97_hl_transformer(int nirows, int nicols)
  {
    // args are anticipated image size
  ...
  FWTSetup fwt2d_make_daub97_lh_transformer(int nirows, int nicols)
  {
    // args are anticipated image size
  ...
  // testing...
  float *fwt2d_get_fkernel(FWTSetup transformer, int *pnrows, int *pncols)
  {
