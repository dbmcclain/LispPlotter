...
#include "nrutil.h"
//
// WATCH OUT... This is from NR in C, and it does not work properly
// Suggest you switch to using LAPACK DGESVD for this stuff...
// DM/RAL 06/07
//
...
extern "C"
long lisp_svdcmp(double *a, int nrow, int ncol, double w[], double *v)
{
  long err = 0;
  ...
  try {
    ...
    ma = convert_dmatrix(a, 1, nrow, 1, ncol);
    mv = convert_dmatrix(v, 1, ncol, 1, ncol);
    
    svdcmp(ma, nrow, ncol, w-1, mv);
  }
  catch(...)
    { err = 1; }
